...
class PasswordChangeAPIView(generics.CreateAPIView):
    ...
    def  create(self,request,*args,**kwargs):
        ...
        if user:
            ...
            return Response({"message":"Password  Changed Succefully"},status=status.HTTP_201_CREATED)
        else :
            return Response({"message":"User not Found or Invalid Otp"},status=status.HTTP_404_NOT_FOUND)
...
class ProfileUpdateAPIView(generics.RetrieveUpdateAPIView):
    ...
    def get_object(self):
        ...
        user=User.objects.get(id=user_id)
        return api_model.Profile.objects.filter(user=user).first()
...
class CartAPIview(generics.CreateAPIView):
    ...
    #The create method is overridden to handle custom creation logic.
    # The method extracts data from the request body: course_id, user_id, price, country_name, and cart_id
    ...
    # It fetches the Course object corresponding to the provided course_id.
    # It fetches the User object if user_id is not "undefined"; otherwise, it sets user to None.


        course=api_model.Course.objects.filter(id=course_id).first()
        
        # price=course.price              # -------------Last Change--Remember----------
        
        if user_id !="undefined":
            user=api_model.User.objects.filter(id=user_id).first()
        else:
            user=None

    # It tries to fetch the Country object based on country_name.
    # If the country is found, it uses its name and tax_rate; otherwise, it defaults to "United States" and a tax_rate of 0.
    ...
    # It checks if a Cart object with the given cart_id and course already exists.

        cart = api_model.Cart.objects.filter(cart_id=cart_id,course=course).first()
        
    # Updating or Creating the Cart:
        ...
        if cart:
            ...
            return Response({"messages":"Cart Updated Successfully"},status=status.HTTP_200_OK)
            ...
            return Response({"messages":"Item added to cart"},status=status.HTTP_201_CREATED)
...
class CartItemDeleteAPIView(generics.DestroyAPIView):
    ...
    def get_object(self):
        ...
        return api_model.Cart.objects.get(cart_id=cart_id,id=item_id)
...
#         order=api_model.CartOrder.objects.get(oid=order_id)
#         coupon=api_model.Coupon.objects.get(code=coupon_code)

#         if coupon:
#             order_item=api_model.CartOrderItem.objects.filter(order=order,teacher=coupon.teacher)
#             for i in order_item:
...
class CouponApplyAPIView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not order_items.exists():
            return Response({"message": "No applicable items for this coupon" ,"icon":"error"}, status=status.HTTP_400_BAD_REQUEST)
...
class StripeCHeckOutAPIView(generics.CreateAPIView):
    ...
    def create(self,request,*args,**kwargs):
        stripe.api_key = settings.STRIPE_SECRET_KEY  # Set the API key here
        ...
        try:
            ...
            return Response({"message":f"something went wrong when trying to make payment: {str(e)}"})
...
#                             type="New Order",
#                         )
#                         if not api_model.EnrolledCourse.objects.filter(course=o.course, user=order.student, teacher=o.teacher, order_item=o).exists():
#                             api_model.EnrolledCourse.objects.create(
#                                 course=o.course,
#                                 user=order.student,
#                                 teacher=o.teacher,
                ...
                if paypal_payment_status == "COMPLETED":
                    if order.payment_status == "Processing":
                        ...
                        for o in order_items:
                            ...
                            # Check if the EnrolledCourse already exists
                            if not api_model.EnrolledCourse.objects.filter(course=o.course, user=order.student, teacher=o.teacher, order_item=o).exists():
                                api_model.EnrolledCourse.objects.create(
                                    course=o.course,
                                    user=order.student,
...
class SearchCourseAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return api_model.Course.filter(title_icontains=query,platform_status="Published", teacher_course_status="Published")
...
class StudentCompletedLessonCreateAPIView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        completed_lesson=api_model.CompletedLesson.objects.filter(user=user,course=course,variant_item=variant_item).first()

        if completed_lesson:
            completed_lesson.delete()
            return Response({"message":"lesson marked as not completed"})
        else:
...
class StudentNoteCreateAPIView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        user=User.objects.get(id=user_id)
        enrollment=api_model.EnrolledCourse.objects.get(enrollment_id=enrollment_id)

        return api_model.Note.objects.filter(user=user,course=enrollment.course)
    ...
    def create(self,request,*args,**kwargs):
        ...
        api_model.Note.objects.create(user=user,course=enrollment.course,title=title,note=note)
...
class StudentNoteDetailAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        note=api_model.Note.objects.get(user=user,course=enrollment.course, id=note_id)

        return note


class StudentRatingAPIView(generics.CreateAPIView):
...
class StudentRatingDetailAPIView(generics.RetrieveUpdateAPIView):
    serializer_class=api_serializer.ReviewSerializer
    permission_classes=[AllowAny]
    ...
    def get_object(self):
        ...
        user=User.objects.get(id=user_id)
        return api_model.Review.objects.get(user=user,review_id=review_id)
...
class StudentWishListAPIView(generics.ListCreateAPIView):
    ...
    def create(self,request,*args, **kwargs):
        ...
        if wishlist:
            ...
            api_model.Wishlist.objects.create(user=user,course=course)
...
class QuestionAnswerListCreateAPIView(generics.ListCreateAPIView):
    ...
    def create(self,request,*args, **kwargs):
        ...
        return Response({"message":"Question Answer Created"})
...
class TeacherSummaryAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        total_revenue=api_model.CartOrderItem.objects.filter(teacher=teacher, order__payment_status="Paid").aggregate(total_revenue=models.Sum("price"))['total_revenue']

        monthly_revenue=api_model.CartOrderItem.objects.filter(teacher=teacher, order__payment_status="Paid",date__gte=one_month_ago).aggregate(total_revenue=models.Sum("price"))['total_revenue'] 

        total_students=api_model.EnrolledCourse.objects.filter(course__teacher=teacher).values('user_id').distinct().count()
...
class TeacherReviewDetailAPIView(generics.RetrieveUpdateAPIView):
    ...
    def get_object(self):
        ...
        review_id=self.kwargs['review_id']
        teacher=api_model.Teacher.objects.get(id=teacher_id)
        return api_model.Review.objects.get(course__teacher=teacher, id=review_id)
...
@api_view(("GET", ))
def TotalMonthlyEarningAPIView(request,teacher_id):
    teacher=api_model.Teacher.objects.get(id=teacher_id)
    ...
    return Response(monthly_earning)
...
class TeacherBestSellingCourseAPIView(viewsets.ViewSet):
    ...
    def list(self,request,teacher_id=None):
        ...
        for course in courses:
            ...
            total_sales=course.enrolledcourse_set.count()
...
# class CourseCreateAPIView(generics.CreateAPIView):
    ...
    #             parts = key.split('[')
    #             variant_index = parts[1].split(']')[0]
    #             field = parts[2].split(']')[0]
    #             item_index = parts[3].split(']')[0] if len(parts) > 3 else None

    #             if variant_index not in variant_data:
    #                 variant_data[variant_index] = {'title': '', 'items': []}
                
    #             if item_index is not None:
    #                 item_index = int(item_index)
    #                 while len(variant_data[variant_index]['items']) <= item_index:
    #                     variant_data[variant_index]['items'].append({})
    #                 variant_data[variant_index]['items'][item_index][field] = value
    #             else:
    #                 variant_data[variant_index][field] = value

    #     for variant_index, data in variant_data.items():
    #         variant = api_model.Variant.objects.create(
    #             title=data['title'],
